...
def get_discovery_targets() -> list[str]:
    ...
    # Explicit master IP for cross-subnet discovery.
    explicit_ip = os.getenv("MASTER_IP", "").strip()
    if explicit_ip:
        add_target(explicit_ip)
        print(f"[Worker] Using explicit MASTER_IP: {explicit_ip}")

    # Try directed broadcast for local /24 network.
    try:
        own_ip = utils.find_own_ip()
        parts = own_ip.split(".")
        if len(parts) == 4:
            directed_bcast = f"{parts[0]}.{parts[1]}.{parts[2]}.255"
            add_target(directed_bcast)
            print(f"[Worker] Own IP: {own_ip}, using directed broadcast: {directed_bcast}")
    except Exception as e:
        print(f"[Worker] Could not determine own IP: {e}")

    add_target(BROADCAST_ADDR)
    print(f"[Worker] Discovery targets: {targets}")
    return targets


def discover_master(sock: socket.socket) -> tuple | None:
    """Broadcasts to find the master and returns its address if found."""
    print("[Worker] Searching for the master...")
    ...
    while True:
        ...
        for target in discovery_targets:
            try:
                ...
                print(f"[Worker] Attempt {attempt}: sent discovery to {target}:{PORT}")
            except Exception as e:
                print(f"[Worker] Error sending to {target}: {e}")
        
        print(f"[Worker] Waiting for ACK...")
        try:
            data, addr = sock.recvfrom(1024)
            if data == ACK_MSG:
                print(f"[Worker] ✓ Got ACK from master at {addr[0]}:{addr[1]}")
                return addr
            else:
                print(f"[Worker] Got unexpected data from {addr}: {data}")
        except socket.timeout:
            print(f"[Worker] Timeout waiting for ACK, will retry in 2 seconds...")
        time.sleep(2)
...
def main():
    ...
    print(f"[Worker] Runtime user: {subprocess.run(['whoami'], capture_output=True, text=True).stdout.strip()}")
    print(f"[Worker] Own IP: {utils.find_own_ip()}")
    print(f"[Worker] Binding UDP socket to port {PORT} for receiving...")
    
    sock = utils.sock_init("", PORT, 'w')
    print(f"[Worker] Socket created with SO_BROADCAST enabled")
    
    try:
        sock.settimeout(2.0)
        print(f"[Worker] Socket timeout: 2.0 seconds")
        
        master_addr = discover_master(sock)
        if not master_addr:
            print("[Worker] ✗ FAILED: Could not find master. Exiting.")
            return

        print(f"[Worker] ✓ Master discovery SUCCESS: {master_addr}")
        sock.settimeout(None) # IMPORTANT: Clear timeout for blocking receives

        # Receive the nodes.txt file from the master
        print("[Worker] Waiting to receive nodes.txt...")
        receive_file(sock, master_addr, NODES_FILENAME)
        print("[Worker] ✓ Received nodes.txt")
        ...
        print(f"[Worker] ✓ Local resources saved to '{resource_file}'")

        print("[Worker] Sending resources file to master...")
        send_file_with_retransmission(sock, master_addr, resource_file)
        print("[Worker] ✓ Resources sent")
    
    except Exception as e:
        print(f"[Worker] ✗ ERROR in main: {e}")
        import traceback
        traceback.print_exc()
    finally:
...
MASTER_IP=192.168.29.188 python3 worker.py
